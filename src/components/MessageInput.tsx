...
import { Send, Smile, Paperclip, Plus, ShieldCheck, Sticker as StickerIcon, Image as ImageIcon, X, Reply, FileIcon, Loader2 } from "lucide-react";
...
export function MessageInput({ 
  ...
  const [loading, setLoading] = useState(false);
    ...
    const { getRootProps, getInputProps, isDragActive } = useDropzone({
      onDrop,
      noClick: true, // We want the buttons to handle clicking
      accept: {
        'image/*': [],
        'application/pdf': [],
        'text/plain': [],
  ...
  const handleSendMessage = async (overrideContent?: string, type: "text" | "image" | "gif" | "sticker" | "file" = "text") => {
    ...
    try {
      ...
      // If we only have files and no text, we send a message for each file or one message with all files
      // Standard behavior is one message with files as payload
